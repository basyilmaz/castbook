...
<div class="row justify-content-center">
    <div class="col-lg-7 col-xl-6">
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <h1 class="h4 mb-1">Yeni Ekstra Alan</h1>
                <p class="text-muted mb-0">Faturalara eklenecek özel alan tanımlayın.</p>
            </div>
        ...
        <div class="card border-0 shadow-sm">
            <form action="{{ route('settings.invoice-extra-fields.store') }}" method="POST">
                @csrf
                <div class="card-body row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="type" name="type" class="form-select @error('type') is-invalid @enderror" required>
                            <option value="text" {{ old('type', 'text') === 'text' ? 'selected' : '' }}>Metin</option>
                            <option value="number" {{ old('type') === 'number' ? 'selected' : '' }}>Sayı</option>
                            <option value="date" {{ old('type') === 'date' ? 'selected' : '' }}>Tarih</option>
                            <option value="select" {{ old('type') === 'select' ? 'selected' : '' }}>Seçim Listesi</option>
                        </select>
                    ...
                    <div class="col-12" id="options-container" style="display: none;">
                        ...
                        <input type="text" id="options" name="options" value="{{ old('options') }}"
